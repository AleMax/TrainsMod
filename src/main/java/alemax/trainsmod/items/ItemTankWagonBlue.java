...
public class ItemTankWagonBlue extends Item {
	
	public ItemTankWagonBlue() {
		setRegistryName("item_tank_wagon_blue");
		setUnlocalizedName("item_tank_wagon_blue");
	...
	public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand,
		...
		if(!worldIn.isRemote) {
			if((worldIn.getBlockState(pos).getBlock() instanceof ITrackBlock || worldIn.getTileEntity(pos) instanceof ITrack) && !(worldIn.getBlockState(pos).getBlock().getRegistryName().equals(ModBlocks.am_rail_curved.getRegistryName()))) {
				EntityTankWagonBlue railcar = new EntityTankWagonBlue(worldIn, pos);
